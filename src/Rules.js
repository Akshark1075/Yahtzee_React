...
class FullHouse extends Rule {
  evalRoll = dice => {
    ...
    // do any of the counts meet of exceed this distro?
    return d.size === 2 && this.freq(dice).includes(3)? this.score : 0;
  };
}
...
class SmallStraight extends Rule {
  evalRoll = dice => {
    const d = new Set(dice);
    var dstr=[...d].sort().join("")
    // Small straight must be 4 different dice 
        
    return d.size >= 4 && (dstr.includes("1234")||dstr.includes("2345")||dstr.includes("3456")) ? this.score : 0;
  };
}
